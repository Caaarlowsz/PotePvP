...
public class Account {
		...
		if (!getPayment().isEmpty()) {
			...
			while (it.hasNext()) {
				...
				if (System.currentTimeMillis() > entry.getValue()) {
					it.remove();
					sendMessage("O seu vip " + Tag.valueOf(entry.getKey().toString()).getPrefix().replace(" ", "")
							+ "�f acabou!");
					sendMessage("Voc� pode adquiri-l� novamente no nosso site: �a�n" + Configuration.SITE.getMessage()
							+ "�f.");
	...
	public void checkRank() {
		...
		if (actualRanking.getXp() < getStatus().getXp()) {
			...
			sendMessage("Voc� subiu para o Ranking �a" + newRanking.name() + "�f.");
			checkRank();
			return;
		}
//		Ranking actualRanking = getRanking();
...
//			Ranking newRanking = Ranking.values()[actualRanking.ordinal() + 1];
//			
//			sendMessage("Voc� upou para o ranking �a" + newRanking.name() + "�f.");
//			setRanking(newRanking);
...
//			sendMessage("Voc� desceu para o ranking �a" + lastedRanking.name() + "�f.");
