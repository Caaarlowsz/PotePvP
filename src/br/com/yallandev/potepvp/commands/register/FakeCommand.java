...
public class FakeCommand extends CommandClass {
	...
	public void onFake(CommandArgs cmdArgs) {
		...
		if (!player.getFakeName().equals(player.getUserName())) {
			player.sendMessage("Voc� j� est� usando um fake!");
			return;
		}
		...
		if (!FakePlayerUtils.validateName(fakeName)) {
			player.sendMessage("O fake �c\"" + fakeName + "\"�f � invalido!");
			return;
		}

		if (fakeName.length() > 16) {
			player.sendMessage("O fake �c\"" + fakeName + "\"�f � muito grande!");
			return;
		}
		if (fakeName.length() < 2) {
			player.sendMessage("O fake �c\"" + fakeName + "\"�f � muito pequeno!");
			return;
		}

		if (isPremium(fakeName)) {
			player.sendMessage(
					"O fake �c\"" + fakeName + "\"�f � pertence a uma conta que j� existe, escolha outro nick!");
			return;
		}
		...
		player.sendMessage("Voc� alterou seu fake �a\"" + fakeName + "\"�f.");
	}
	...
	public void fakePlayer(Account p, EntityPlayer entityPlayer, String fake) {
		...
		Player skinPlayer = p.getPlayer().getWorld().getPlayers()
				.get(new Random().nextInt(p.getPlayer().getWorld().getPlayers().size()));
