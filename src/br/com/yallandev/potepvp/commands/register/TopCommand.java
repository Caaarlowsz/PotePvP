...
public class TopCommand extends CommandClass {
	...
	public void onTop(CommandArgs cmdArgs) {
		...
		if (!a[0].equalsIgnoreCase("kills") && !a[0].equalsIgnoreCase("deaths") && !a[0].equalsIgnoreCase("killstreak")) {
			send(p, "Use §a/top [Kills/Deaths/KillStreak] [Numero]§f para ordernar um top.");
			return;
		}
		...
		if (order >= 200) {
			send(p, "O numero não pode ser maior que 200!");
			return;
		}
		
		if (order <= 0) {
			send(p, "O numero não pode ser menor do que 1!");
			return;
		}
	...
	public List<String> getTop(String type, int max) {
		...
		try {
			PreparedStatement stm = BukkitMain.getConnection().getConnection().prepareStatement("SELECT * FROM `kitpvp_status` ORDER BY `" + type +"` DESC LIMIT 0,"+max+"");
			...
			while (rs.next()) {
				...
				i++;
				tops.add("§a" + i + "º §6" + player.getUserName() + " §8|§a " + rs.getInt(type) + " " + type.substring(0, 1).toUpperCase() + type.substring(1, type.length()).toLowerCase());
			}
