	...
	public void onTell(CommandArgs cmdArgs) {
		...
		if (a.length == 1) {
			if ((a[0].equalsIgnoreCase("on"))) {
				if (recieve.contains(p)) {
					send(p, "Você pode receber mensagem de tell!");
					recieve.remove(p);
					return;
				}
				send(p, "Você já está com o tell ativado!");
				return;
			}
			if ((a[0].equalsIgnoreCase("off"))) {
				if (!recieve.contains(p)) {
					send(p, "Você não pode receber mensagem de tell!");
					recieve.add(p);
					return;
				}
				send(p, "Você já está com o tell desativado!");
				return;
			}
		...
		if (a.length > 1) {
			...
			if (recieve.contains(r)) {
				send(p, "Você não pode mandar mensagem para esse jogador, porque ele está com o tell desativado!");
				return;
			}
			String msg = "";
			...
			p.sendMessage("§7[Você -> " + r.getName() + "] " + msg);
			r.sendMessage("§7[" + p.getName() + " -> Você]" + msg);
			return ;
		}
		send(p, "Use §a/tell [on/off]§f para desativar ou ativar o tell!");
		send(p, "Use §a/tell [Player] [Mensagem]§f para mandar uma mensagem!");
