	...
	public void onRank(CommandArgs cmdArgs) {
		for (Ranking ranks : Ranking.values()) {
			if (ranks == Ranking.POTTER) {
				...
				chat.setHoverEvent(new HoverEvent(Action.SHOW_TEXT,
						new ComponentBuilder("Ultimo rank, o rank dos vencedores!").create()));
				cmdArgs.getPlayer().spigot().sendMessage(chat);
			} else {
				TextComponent chat = new TextComponent(ranks.getColor() + ranks.getIcon() + " " + ranks.name());
				chat.setHoverEvent(new HoverEvent(Action.SHOW_TEXT,
						new ComponentBuilder("Voc� precisa de " + ranks.getXp()
								+ " e para sair\n�fdesse rank e upar para �a" + Ranking.values()[ranks.ordinal() + 1])
										.create()));
				cmdArgs.getPlayer().spigot().sendMessage(chat);
			}
		}
		...
		cmdArgs.getAccount().sendMessage("Voc� est� no rank " + player.getRanking().getColor()
				+ player.getRanking().getIcon() + " " + player.getRanking().name() + "�f.");
		cmdArgs.getAccount().sendMessage("Voc� est� com �a" + player.getStatus().getXp() + "�f.");
