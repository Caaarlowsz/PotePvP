...
public class GroupCommand extends CommandClass {
	...
	public void onGroupset(CommandArgs cmdArgs) {
		...
		if (uuid == null) {
			send(sender, "O jogador §c\"" + a[0] + "\" §fnão existe no nosso banco de dados!");
			return;
		}
		...
		if (player.getServerGroup().ordinal() >= Group.ADMIN.ordinal()) {
			if (!hasServerGroup(sender, Group.DONO)) {
				send(sender, "Você não pode manejar o grupo desse jogador!");
				return;
			}
		}
		...
		if (group.ordinal() >= Group.GERENTE.ordinal()) {
			if (!hasServerGroup(sender, Group.DONO)) {
				send(sender, "Você não pode manejar esse grupo!");
				return;
			}
		}
		...
		if (actualGroup == group) {
			send(sender, "O jogador §a\"" + player.getUserName() + "\"§f já está no grupo §a§l" + group.name() + "§f.");
			return;
		}
		...
		if (group == Group.MEMBRO) {
			...
			player.sendMessage("Seu grupo foi alterado para §a" + (Tag.valueOf(group.name()) == Tag.MEMBRO ? "§f§lMEMBRO" : Tag.valueOf(group.name()).getPrefix().replace(" ", "") )+ "§f.");
		}
		...
		send(sender, "Você alterou o grupo do §a\"" + player.getUserName() + "\"§f para §a§l" + group.name() + "§f.");
		broadcast("O grupo do jogador §a" + player.getUserName() + "§f foi alterado para §a" + group.name() + "§f.", Group.MOD);
	}
	...
	public void onTempgroup(CommandArgs cmdArgs) {
		...
		} catch (Exception e) { send(sender, "Use um formato de tempo valido! (Exemplo: 30s, 30m, 30d, 30w, 30mo, 30y)");}
		...
		player.getPayment().put(group, player.getPayment().containsKey(group) ? player.getPayment().get(group) + newTime : expire);
		player.sendMessage("Você ganhou §a" + group.name() + "§f por §a" + DateUtils.getTime(expire) + "§f.");
		
		if (!player.isOnline()) {
			BukkitMain.getAccountCommon().saveAccount(player);
		}
		
		send(sender, "Você deu setou temporariamente o grupo §a" + group.name() + "§f para o jogador §a" + player.getUserName() + "§f por §a" + DateUtils.getTime(expire) + "§f.");
		broadcast("O grupo §a" + group.name() + "§f foi temporariamente setado para o jogador §a" + player.getUserName() + "§f por §a" + DateUtils.getTime(expire) + "§f.", Group.MOD);
	}
	...
	public void onRemovervip(CommandArgs cmdArgs) {
		...
		if (!player.getPayment().containsKey(group)) {
			send(sender, "O jogador §a\"" + player.getUserName() + "\"§f não possui o grupo §a" + group.name() + "§f.");
			return;
		}
		...
		send(sender, "O grupo " + Tag.valueOf(a[0]).getPrefix().replace(" ", "") + "§f do jogador §a" + player.getUserName() + "§f foi retirado!");
		broadcast("O grupo §a" + group.name() + "§f foi retirado do jogador §a" + player.getUserName() + "§f.", Group.MOD);
	}
