...
public class AccountCommand extends CommandClass {
	...
	public void onList(CommandArgs cmdArgs) {
		cmdArgs.getSender().sendMessage("§6§l» §e" + Bukkit.getOnlinePlayers().length + "/" + Bukkit.getMaxPlayers() + " jogadores");
	}
	...
	public void onAccount(CommandArgs cmdArgs) {
		...
		s.sendMessage(CenterChat.centered("§eGrupo: §3" + (player.getGroup() == Group.MEMBRO ? "§f§lMEMBRO" : Tag.valueOf(player.getGroup().name()).getPrefix().replaceAll(" ", ""))));
		s.sendMessage(CenterChat.centered("§eRank: §3" + player.getRanking().getColor() + player.getRanking().getIcon() + " " + player.getRanking().name()));
		s.sendMessage(CenterChat.centered(""));
	...
	public void open(Account player, Player p) {
		...
		if (status == null) {
			send(p, "Não foi possivel localizar o status desse jogador!");
			return;
		}
