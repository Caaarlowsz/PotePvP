	...
	public void onEvento(CommandArgs cmdArgs) {
		...
		if (a.length == 0) {
			if (main.getEventManager().isRunningRDM()) {
				...
				send(s, "N�o h� nenhum evento no momento!");
			}
					...
					if (main.getEventManager().isRunningRDM()) {
						...
						if (main.getEventManager().getRdmAutomatic().getPlayers().size() > main.getEventManager()
							...
							if (!hasServerGroup(s, Group.MVP)) {
								send(s, "O evento est� cheio!");
								send(s, "Compre vip em �n" + Configuration.SITE.getMessage()
										+ "�f e tenha o acesso liberado!");
								return;
							}
						}
			...
			if (a[0].equalsIgnoreCase("sair")) {
				...
				if (hasServerGroup(s, Group.MODPLUS)) {
					if (main.getEventManager().isRunningRDM()) {
						send(s, "J� h� um evento em andamento!");
					} else {
						main.getEventManager().setRdmAutomatic(new RDMAutomatic());
						send(s, "Voc� iniciou um evento rei da mesa!");
				...
				if (hasServerGroup(s, Group.MODPLUS)) {
					if (main.getEventManager().isRunningRDM()) {
						...
						send(s, "Voc� finalizou o evento �aRei da Mesa�f.");
						broadcast("O jogador �a" + s.getName() + "�f fechou um evento �aRei da Mesa�f.", Group.TRIAL);
					} else {
						send(s, "N�o h� nenhum evento em andamento no momento!");
					}
				}
			}
		} else {
			if (a[0].equalsIgnoreCase("time")) {
				if (hasServerGroup(s, Group.MODPLUS)) {
					...
					send(s, "Voc� alterou o tempo do evento para �a" + time + "�f.");
					broadcast("O tempo do evento foi alterado pelo �a" + s.getName() + "�f para �a" + time + "�f.",
							Group.TRIAL);
				}
				...
				if (hasServerGroup(s, Group.MODPLUS)) {
					...
					send(s, "Voc� alterou o maximo de jogadores do evento para �a" + time + "�f.");
					broadcast("O maximo de jogadores do evento foi alterado pelo �a" + s.getName() + "�f para �a" + time
							+ "�f.", Group.TRIAL);
				}
			}
		}
