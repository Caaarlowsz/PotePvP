...
public class TeleportCommand extends CommandClass {
	...
	public void onTeleportall(CommandArgs cmdArgs) {
		...
		if (a.length == 0) {
			...
			send(p, "Todos os jogadores foram teletransportados para voc�!");
		...
		if (target == null) {
			send(p, "O jogador �c\"" + a[0] + "\"�f n�o est� online!");
			return;
		}
		...
		broadcast("Todos os jogadores foram for�ados a ser teletransportados para o �a" + target.getName()
				+ "�f pelo �a" + p.getName() + "�f.", Group.MOD);
		broadcast("Todos os jogadores foram teletransportados!");
	}
