...
//	private HashMap<String, Kit> playerKit;
	...
	public Status loadStatus(UUID uuid) {
		...
		try {
			...
			PreparedStatement stmt = connection.prepareStatement("SELECT * FROM `kitpvp_status` WHERE `Uuid`='" + uuid.toString() + "';");
			ResultSet result = stmt.executeQuery();
			if (result.next()) {
				Status player = new Status(uuid, result.getInt("kills"), result.getInt("deaths"), result.getInt("killstreak"), result.getInt("money"), result.getInt("xp"), result.getInt("highestKS"));
	...
	public Status loadStatus(UUID uuid, boolean value) {
		...
		try {
			...
			if (result.next()) {
				status = new Status(uuid, result.getInt("kills"), result.getInt("deaths"), result.getInt("killstreak"), result.getInt("money"), result.getInt("xp"), result.getInt("highestKS"));
			}
	...
	public Status loadStatus(UUID uuid, Status status) {
		if (this.playerStatus.containsKey(uuid))
			return this.playerStatus.get(status);
	...
	public void saveStatus(UUID uuid, Status status) {
		...
		try {
			PreparedStatement stmt = BukkitMain.getConnection().prepareStatment("INSERT INTO `kitpvp_status`(`Uuid`, `kills`, `deaths`, `killstreak`, `money`, `xp`, `highestKS`) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE `kills`=VALUES(`kills`), `deaths`=VALUES(`deaths`), `killstreak`=VALUES(`killstreak`), `money`=VALUES(`money`), `xp`=VALUES(`xp`), `highestKS`=VALUES(`highestKS`);");
	...
	public void setProtection(UUID uuid, Boolean value) {
		...
		if (Bukkit.getPlayer(uuid) != null) {
			if (value)
				Bukkit.getPlayer(uuid).sendMessage(Configuration.PREFIX.getMessage() + "Você recebeu a proteção do spawn!");
			else
				Bukkit.getPlayer(uuid).sendMessage(Configuration.PREFIX.getMessage() + "Sua proteção foi retirada!");
		}
	}
