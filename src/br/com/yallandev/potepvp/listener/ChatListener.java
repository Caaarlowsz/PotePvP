...
public class ChatListener implements Listener {
	...
	public void onChatEnabled(AsyncPlayerChatEvent event) {
		...
		if (!player.hasServerGroup(Group.TRIAL)) {
			if (chatCooldown.containsKey(player.getUserName())) {
				if (chatCooldown.get(player.getUserName()) > System.currentTimeMillis()) {
					player.sendMessage("Voc� deve esperar mais �a"
							+ DateUtils.getTime(chatCooldown.get(player.getUserName())) + "�f para falar no chat!");
					event.setCancelled(true);
					return;
				}
			}
		...
		if (event.isCancelled()) {
			player.sendTimerMessage("O chat est� �4�lDESATIVADO�f.");
		}
		p = null;
		player = null;
	}
	...
	public void onChat(AsyncPlayerChatEvent event) {
		...
		event.setFormat((player.hasClan() && !player.hasFake()
				? "�7[" + player.getClan().getClanTag().replace("&", "�") + "�7] "
				: "")
				+ event.getPlayer().getDisplayName()
				+ (!player.hasFake() ? " �7(" + player.getRanking().getColor() + player.getRanking().getIcon() + "�7)"
						: " �7(" + Ranking.UNRANKED.getColor() + Ranking.UNRANKED.getIcon() + "�7)")
				+ "�6�l� �f"
				+ (player.hasServerGroup(Group.HEIGHT) ? event.getMessage().replace("&", "�") : event.getMessage()));
	}
}
