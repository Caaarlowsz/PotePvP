...
public class PlayerListener implements Listener {
	...
	public void onPlayerLogin(PlayerLoginEvent e) {
		...
		if (Bukkit.getPlayer(userName) != null) {
			if (Bukkit.getPlayer(userName).getAddress().getAddress().getHostAddress()
					.equals(e.getAddress().getHostAddress())) {
				Bukkit.getPlayer(userName).kickPlayer("Voc� est� conectando no servidor!");
			}

			e.disallow(Result.KICK_OTHER,
					Configuration.KICK_PREFIX.getMessage() + "\n�cEst� conta j� est� logada no servidor!");
			return;
		}

		if (e.getResult() == Result.KICK_WHITELIST) {
			e.setKickMessage(Configuration.KICK_PREFIX.getMessage()
					+ "\n�cO servidor est� em manuten��o!\nEstamos trabalhando para sua divers�o!");
						...
						BukkitMain.getInstance().getBanManager().ban(player,
								new Ban(ipBan.getValue().getBannedBy(),
										(ipBan.getValue().getReason().contains(" [ALTERNATIVE ACCOUNT]")
												...
												: ipBan.getValue().getReason() + " [ALTERNATIVE ACCOUNT]")));
				}
			}
		}
	}
	...
	public void onPlayerJoin(PlayerJoinEvent e) {
		...
		for (String list : Configuration.JOIN_MESSAGES.getStringList()) {
			...
			p.sendMessage((parsed[0].equals("true")
					? CenterChat
							.centered(parsed[1].replace("&", "�").replace("{site}", Configuration.SITE.getMessage()))
					: parsed[1].replace("&", "�").replace("{site}", Configuration.SITE.getMessage())));
		}
		...
		if (player.getServerVersion() == ServerVersion.NONE) {
			player.sendMessage("Por motivo de muitos bug's em nosso sistema de cl�, tivemos que resetar!");
			player.sendMessage(
					"Mesmo que voc� n�o tenha criado um cl�, voc� recebeu �a15000�f de coins para criar um cl�!");
			player.sendMessage("N�s, da staff, sentimos muito pelo ocorrido!");
	...
	public void onPlayerDamageByEntity(EntityDamageByEntityEvent event) {
		...
		main.getPlayerManager().setCombatLog(damager.getUuid(),
	...
	public void onEntityDamage(EntityDamageEvent e) {
		...
		if (!main.getServerManager()
				.isDamageEnable(main.getPlayerManager().getWarp(((Player) e.getEntity()).getUniqueId()))
			...
			if (main.getKitManager().hasAbility(player.getUuid(), "Nenhum")) {
				...
				for (ItemStack item : main.getKitManager().getAbility(player.getUuid()).getKitItens()) {
					if (e.getItemDrop().getItemStack().getType() == item.getType()) {
						e.setCancelled(true);
						player.sendAction("�cVoc� n�o pode dropar o item do seu kit!");
						return;
					}
				}
	...
	public void onPlayerDeath(PlayerDeathEvent e) {
		...
		if (!main.getPlayerManager().getWarp(p.getUniqueId()).getWarpName().equalsIgnoreCase("Lava")) {
			...
			player.saveStatus();
			player.sendMessage("Voc� perdeu �c3 xp�f.");
			player.sendMessage("Voc� perdeu �c50 coins�f.");
		}
		...
		if (p.getKiller() != null) {
			...
			killer.sendMessage("Voc� matou o jogador �a" + p.getName() + "�f.");
			killer.sendMessage("Voc� recebeu �a" + randomXp * (killer.multiplier()) + " xp�f.");

			if (killer.multiplier() != 1) {
				killer.sendMessage("�fVoc� ganhou �b�lXP�b em " + killer.multiplier() + "X�f.");
			}

			killer.sendMessage("Voc� recebeu �a" + randomMoney * (killer.multiplier()) + " coins�f.");

			if (killer.multiplier() != 1) {
				killer.sendMessage("�fVoc� ganhou �b�lMONEY�b em " + killer.multiplier() + "X�f.");
			}
		...
		if (!main.getPlayerManager().getWarp(p.getUniqueId()).getWarpName().equalsIgnoreCase("Spawn")) {
			PlayerDeathInWarpEvent event = new PlayerDeathInWarpEvent(p,
					main.getPlayerManager().getWarp(p.getUniqueId()));
	...
	public void onCommand(PlayerCommandPreprocessEvent event) {
		...
		if (event.getMessage().split(" ")[0].contains(":")) {
			event.getPlayer().sendMessage(
					Configuration.PREFIX.getMessage() + "Voc� n�o pode usar comando que tenham �a\":\"�f.");
			event.setCancelled(true);
			return;
		}
	}
