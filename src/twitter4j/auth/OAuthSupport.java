...
public interface OAuthSupport {
	 ...
	 * @throws IllegalStateException when OAuth consumer has already been set, or
	 *                               the instance is using basic authorization
	 * @since Twitter 2.0.0
	 */
	 ...
	 * @see <a href=
	 *      "https://dev.twitter.com/docs/api/1.1/post/oauth/request_token">POST
	 ...
	 *
	 * @param callbackURL     callback URL
	 * @param xAuthAccessType Overrides the access level an application requests to
	 *                        a users account. Supported values are read or write.
	 *                        This parameter is intended to allow a developer to
	 *                        register a read/write application but also request
	 ...
	 *                        register a read/write application but also request
	 *                        read only access when appropriate.
	 * @param xAuthMode       Set to reverse_auth to obtain a special request token
	 *                        to be used in the reverse auth process.
	 * @return generated request token
	 * @throws TwitterException      when Twitter service or network is unavailable
	 * @throws IllegalStateException access token is already available
	 * @see <a href="https://dev.twitter.com/docs/auth/oauth/faq">OAuth FAQ |
	 ...
	 * @since Twitter4J 2.2.3
	 */
	RequestToken getOAuthRequestToken(String callbackURL, String xAuthAccessType, String xAuthMode)
			throws TwitterException;

	/**
	 * Returns an access token associated with this instance.<br>
	 * If no access token is associated with this instance, this will retrieve a new
	 * access token.
	 *
	 * @return access token
	 * @throws TwitterException      when Twitter service or network is unavailable,
	 ...
	 * @see <a href=
	 *      "https://dev.twitter.com/docs/api/1.1/post/oauth/access_token">POST
	...
	/**
	 * Retrieves an access token associated with the supplied request token and sets
	 * userId.
	 *
	 ...
	 * sending an email to api@twitter.com — all other applications will receive an
	 * HTTP 401 error. Web-based applications will not be granted access, except on
	 * a temporary basis for when they are converting from basic-authentication
	 * support to full OAuth support.<br>
	 * Storage of Twitter usernames and passwords is forbidden. By using xAuth, you
	 * are required to store only access tokens and access token secrets. If the
	 * access token expires or is expunged by a user, you must ask for their login
	 * and password again before exchanging the credentials for an access token.
	 *
