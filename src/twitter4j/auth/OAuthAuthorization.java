...
public class OAuthAuthorization implements Authorization, java.io.Serializable, OAuthSupport {
	...
	public RequestToken getOAuthRequestToken(String callbackURL, String xAuthAccessType, String xAuthMode)
		...
		oauthToken = new RequestToken(
				http.post(conf.getOAuthRequestTokenURL(), params.toArray(new HttpParameter[params.size()]), this, null),
				this);
	...
	public AccessToken getOAuthAccessToken(String screenName, String password) throws TwitterException {
		try {
			...
			oauthToken = new AccessToken(http.post(url,
	 ...
	 */
	/* package */ String generateAuthorizationHeader(String method, String url, HttpParameter[] params,
			OAuthToken token) {
	 ...
	 * 1. Parameters are sorted by name, using lexicographical byte value ordering.
	 ...
	 * 3. <br>
	 * 4. Parameters are concatenated in their sorted order into a single string.
	 * For each parameter, the name is separated from the corresponding value by an
	 * ‘=’ character (ASCII code 61), even if the value is empty. Each name-value
	 * pair is separated by an ‘&’ character (ASCII code 38). For example:<br>
	 ...
	 * signature to a specific endpoint. The URL used in the Signature Base String
	 * MUST include the scheme, authority, and path, and MUST exclude the query and
	 * fragment as defined by [RFC3986] section 3.<br>
	 * If the absolute request URL is not available to the Service Provider (it is
	 * always available to the Consumer), it can be constructed by combining the
	 * scheme being used, the HTTP Host header, and the relative HTTP request URL.
	 * If the Host header is not available, the Service Provider SHOULD use the host
	 * name communicated to the Consumer in the documentation or other means.<br>
	 * The Service Provider SHOULD document the form of URL used in the Signature
	 * Base String to avoid ambiguity due to URL normalization. Unless specified,
	 * URL scheme and authority MUST be lowercase and include the port number; http
	 * default port 80 and https default port 443 MUST be excluded.<br>
	 * <br>
