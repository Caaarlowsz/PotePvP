...
public interface PlacesGeoResources {
	 ...
	 * {@link PlacesGeoResources#reverseGeoCode(twitter4j.GeoQuery)} method. <br>
	 ...
	 * @see <a href="https://dev.twitter.com/docs/api/1.1/get/geo/id/:place_id">GET
	 ...
	 * of places, have the user validate the location he or she is at, and then send
	 * the ID of this location up with a call to statuses/update.<br>
	 ...
	 * This API call is meant to be an informative call and will deliver generalized
	 * results about geography. <br>
	 ...
	 * Conceptually, a query can be made from the user's location, retrieve a list
	 * of places, have the user validate the location he or she is at, and then send
	 * the ID of this location with a call to statuses/update. <br>
	 ...
	 * choose from first. Then, if the desired place doesn't exist, make a request
	 * to post/geo/place to create a new one. <br>
	 * The token contained in the response is the token needed to be able to create
	 * a new place. <br>
	 ...
	 *                        restrict the search results to. Setting this value
	 *                        means only places within the given place_id will be
	 *                        found.
	 * @param streetAddress   optional: This parameter searches for places which
	 *                        have this given street address. There are other
	 *                        well-known, and application specific attributes
	 *                        available. Custom attributes are also permitted. Learn
	 *                        more about Place Attributes.
	 * @return places (cities and neighborhoods) that can be attached to a
	 *         statuses/update
	 * @throws TwitterException when Twitter service or network is unavailable
	 * @since Twitter4J 2.1.7
