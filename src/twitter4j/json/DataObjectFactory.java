...
public final class DataObjectFactory {
	 ...
	 * Note that raw JSON forms can be retrieved only from the same thread invoked
	 * the last method call and will become inaccessible once another method call
	 *
	...
	/**
	 * Construct an object from rawJSON string. This method may be called when you
	 * do not know what a given raw JSON string contains. It will do the work of
	 * determining what type of object the JSON represents, and constructing the
	 * respective object type. For example, if the JSON contents represents a
	 * Status, then a Status will be returned. If it represents a deletion notice,
	 * then a StatusDeletionNotice will be returned. The caller can simply use
	 * instanceof to handle the returned object as applicable. NOTE: the raw
	 * JSONObject will be returned in cases where there isn't a discrete respective
	 * object type that can be constructed. That way, the caller can at least have
	 * access to the JSON itself.
	 *
	 * @param rawJSON raw JSON form as String
	 ...
	 * @throws TwitterException when provided string is not a valid JSON string.
	 * @since Twitter4J 2.1.9
	 * @deprecated use {@link twitter4j.TwitterObjectFactory#createObject(String)}
	 *             instead
	 */
