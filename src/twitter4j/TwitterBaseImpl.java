...
abstract class TwitterBaseImpl
		implements TwitterBase, java.io.Serializable, OAuthSupport, OAuth2Support, HttpResponseListener {
	...
	public String getScreenName() throws TwitterException, IllegalStateException {
		...
		if (null == screenName) {
			...
			if (null == screenName) {
				// retrieve the screen name if this instance is authenticated with OAuth or
				// email address
	...
	User fillInIDAndScreenName() throws TwitterException {
		...
		User user = new UserJSONImpl(
				http.get(conf.getRestBaseURL() + "account/verify_credentials.json", null, auth, this), conf);
	...
	/**
	 * {@inheritDoc} Basic authenticated instance of this class will try acquiring
	 * an AccessToken using xAuth.<br>
	 * In order to get access acquire AccessToken using xAuth, you must apply by
	 * sending an email to <a href="mailto:api@twitter.com">api@twitter.com</a> all
	 * other applications will receive an HTTP 401 error. Web-based applications
	 * will not be granted access, except on a temporary basis for when they are
	 * converting from basic-authentication support to full OAuth support.<br>
	 ...
	 *
	 * @throws TwitterException When Twitter service or network is unavailable, when
	 *                          the user has not authorized, or when the client
