...
public class PingAnimation {
	...
	/**
	 * Constructs an animation of ping packets to send to the client. The supplied
	 * String arrays hold the actual animation (color codes are translated). Each
	 * step uses the next value, looping back to the start after it reached the end.
	 * Note that it will keep running until the supplied step amount is reached,
	 * even if the client closes the server list.
	 * 
	 * @param plugin       Instance of your plugin.
	 * @param event        PingEvent to animate
	 * @param stepAmount   The max amount of steps, after which the animation will
	 *                     stop.
	 * @param ticksPerStep Amount of ticks to wait between steps
	 * @param motd         An array of Strings to animate the MOTD with. Each step
	 *                     uses the next value, looping back to the start after it
	 *                     reached the end.
	 */
	...
	private class Animation extends BukkitRunnable {
		...
		void animate() {
			...
			if (protocolAnimation != null)
				protocol = ChatColor.translateAlternateColorCodes('&',
						protocolAnimation[step % protocolAnimation.length]);
		}
